...
public:
    vector<int> spiralOrder(vector<vector<int>> &matrix)
    {
        ...
        int m = matrix.size(), n = matrix.size() - 1, techomtrx = 0, pisomtrx = m - 1, izqMtrx = 0, derMtrx = n - 1, indx = 0;
        ...
        while (techomtrx <= pisomtrx && izqMtrx <= derMtrx)
        {
            ...
            for (int columna = derMtrx; columna >= 1; --columna){
                matriz[indx++] = matrix[derMtrx][columna];
            }
            if()
        }
    }
